...
public class CamposGolfDAO {
    public void getCampos(ArrayList<CampoGolf> lista) {
		...
		try (PreparedStatement pst = con.prepareStatement("SELECT * FROM campos_golf");
            ...
            while (rs.next()) {
            	lista.add(new CampoGolf(rs.getString(1), rs.getString(2), rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6)));
            }
	...
	public CampoGolf getCampo(int id) {
		...
		try (PreparedStatement pst = con.prepareStatement(consulta)) {
			...
			try (ResultSet rs = pst.executeQuery()) {
				...
				if (rs.next()) {
					cu = new CampoGolf(rs.getString(1), rs.getString(2), rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6));  // Obtener los datos de la fila resultante
				}
			}
