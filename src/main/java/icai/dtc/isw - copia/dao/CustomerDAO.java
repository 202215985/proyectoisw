...
public class CustomerDAO {
	...
	public Usuario getCliente(String dni) {
		...
		try (PreparedStatement pst = con.prepareStatement(consulta)) {
			...
			try (ResultSet rs = pst.executeQuery()) {
				...
				if (rs.next()) {
					cu = new Usuario(rs.getString(1), rs.getString(2), rs.getString(3),rs.getString(4),rs.getString(5));  // Obtener los datos de la fila resultante
				}
			}
	...
	public Usuario login(String dni, String password) {
		...
		try (PreparedStatement pst = con.prepareStatement(consulta)) {
			// Asignar el valor del parámetro
			pst.setString(1, dni);
			pst.setString(2, password);  // El primer parámetro "?" se reemplaza por el valor de 'id'
	...
	public void setCliente(Usuario usu) {
		Connection con=ConnectionDAO.getInstance().getConnection();
		String insertar = "INSERT INTO usuarios (dni, nombre, apellidos, correo, password) VALUES (?, ?, ?, ?, ?)";
